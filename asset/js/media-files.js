...
(function ($) {
	...
	$(document).ready(function () {
		...
		determine_media_to_migrate = function () {
			...
			$.ajax({
				...
				error: function (jqXHR, textStatus, errorThrown) {
					...
					$('.progress-text').html(wpsdbmf_strings.error_determining + ' (#101mf)');
		...
		media_successfully_determined = function (data) {
			...
			$('.progress-tables').prepend('<div title="' + wpsdbmf_strings.media_files + '" style="width: 100%;" class="progress-chunk media_files"><span>' + wpsdbmf_strings.media_files + ' (<span class="media-migration-current-image">0</span> / ' + wpsdb_add_commas(Object.size(args.files_to_migrate)) + ')</span></div>');
		...
		function migrate_media_files_recursive(args) {
			...
			$.ajax({
				...
				error: function (jqXHR, textStatus, errorThrown) {
					...
					$('.progress-text').html(wpsdbmf_strings.problem_migrating_media + ' (#102mf)');
					$('.progress-text').addClass('migration-error');
					console.log(jqXHR);
					console.log(textStatus);
					console.log(errorThrown);
				...
				success: function (data) {
					...
					$('.progress-text').html(overall_percent + '% - ' + wpsdbmf_strings.migrating_media_files);
