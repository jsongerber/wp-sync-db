...
(function ($) {
	...
	function setup_counter() {
		(timer_count = 0), (counter_display = $('.timer')), (label = wpsdb_i10n.time_elapsed + ' ');
	...
	function maybe_show_version_warning(plugin_version, url) {
		if (typeof plugin_version != 'undefined' && plugin_version != wpsdb_plugin_version) {
	...
	$(document).ready(function () {
		$('#plugin-compatibility').change(function (e) {
			...
			if ($(this).is(':checked')) {
				...
				var answer = confirm('If confirmed we will install an additional WordPress "Must Use" plugin. This plugin will allow us to control which plugins are loaded during WP Sync DB specific operations. Do you wish to continue?');
			...
			$(this)
				.parent()
				.append('<img src="' + spinner_url + '" alt="" class="ajax-spinner general-spinner" />');
		...
		$('.amount').html(wpsdb_add_commas($('.slider').slider('value')) + ' kB');
		...
		function establish_remote_connection_from_saved_profile() {
			...
			if (typeof wpsdb_default_profile == 'undefined' || wpsdb_default_profile == true || action == 'savefile' || doing_ajax) {
				return;
			}
			...
			$.ajax({
				...
				error: function (jqXHR, textStatus, errorThrown) {
					$('.connection-status').html(wpsdb_i10n.connection_local_server_problem + ' (#102)');
				...
				success: function (data) {
					...
					var loaded_tables = '';
					if (wpsdb_default_profile == false && typeof wpsdb_loaded_tables != 'undefined') {
					...
					if (wpsdb_default_profile == false && typeof wpsdb_loaded_post_types != 'undefined') {
					...
					$.each(connection_data.post_types, function (index, value) {
						...
						if (selected != -1 || (wpsdb_convert_exclude_revisions == true && value != 'revision')) {
							selected = ' selected="selected" ';
						} else {
							selected = ' ';
						}
					...
					if (wpsdb_default_profile == false && typeof wpsdb_loaded_tables_backup != 'undefined') {
					...
					$.wpsdb.do_action('verify_connection_to_remote_site', connection_data);
				},
			});
		}
		...
		$('.migrate-db-button').click(function (event) {
			...
			if (migration_intent == 'savefile') {
				...
				static_migration_label = get_default_profile_name(remote_site, migration_intent, true) + ', ' + wpsdb_i10n.please_wait;
				completed_msg = get_default_profile_name(remote_site, migration_intent, true) + ' ' + wpsdb_i10n.complete;
			}
			...
			if (stage == 'backup') {
				...
				if (migration_intent == 'push') {
					...
					if (backup_option == 'backup_only_with_prefix') {
						...
						selected_tables = $('#select-tables').val();
						tables_to_migrate = get_intersect(selected_tables, wpsdb_this_tables);
					} else if (backup_option == 'backup_manual_select') {
						tables_to_migrate = $('#select-backup').val();
					}
				}
				...
				if (table_intent == 'migrate_select') {
					// user has elected to migrate only certain tables
			...
			$.ajax({
				...
				error: function (jqXHR, textStatus, errorThrown) {
					...
					$('.progress-text').html(wpsdb_i10n.connection_local_server_problem + ' (#112)');
					...
					migrate_table_recursive = function (current_row, primary_keys) {
						if (i >= tables_to_migrate.length) {
							if (stage == 'backup') {
								...
								hooks = $.wpsdb.apply_filters('wpsdb_before_migration_complete_hooks', hooks);
								hooks.push('migration_complete');
								hooks = $.wpsdb.apply_filters('wpsdb_after_migration_complete_hooks', hooks);
						...
						if (stage == 'backup') {
							$('.progress-text').html(overall_percent + '% - ' + wpsdb_i10n.backing_up + ' "' + tables_to_migrate[i] + '"');
						} else {
							$('.progress-text').html(overall_percent + '% - ' + wpsdb_i10n.migrating + ' "' + tables_to_migrate[i] + '"');
						}
						...
						$.ajax({
							...
							error: function (jqXHR, textStatus, errorThrown) {
								$('.progress-title').html('Migration failed');
								$('.progress-text').html(wpsdb_i10n.table_process_problem + ' ' + tables_to_migrate[i]);
								$('.progress-text').append('<br /><br />' + wpsdb_i10n.status + ': ' + jqXHR.status + ' ' + jqXHR.statusText + '<br /><br />' + wpsdb_i10n.response + ':<br />' + jqXHR.responseText);
								$('.progress-text').addClass('migration-error');
								doing_ajax = false;
								console.log(jqXHR);
								console.log(textStatus);
							...
							success: function (data) {
								...
								try {
									...
									$('.progress-title').html('Migration failed');
									if ('' == data || null == data) {
										$('.progress-text').html(wpsdb_i10n.table_process_problem_empty_response + ' ' + tables_to_migrate[i]);
									} else {
								...
								if (typeof row_information.wpsdb_error != 'undefined' && row_information.wpsdb_error == 1) {
									$('.progress-title').html('Migration failed');
									$('.progress-text').addClass('migration-error');
		...
		migration_complete = function () {
			...
			if (migration_intent == 'savefile') {
				...
				if ($('#save_computer').is(':checked')) {
					var url = wpsdb_this_download_url + encodeURIComponent(dump_filename);
					if ($('#gzip_file').is(':checked')) {
						url += '&gzip=1';
					}
					window.location = url;
				} else {
					migrate_complete_text = wpsdb_i10n.completed_dump_located_at + ' <a href="' + dump_url + '">' + dump_url + '</a>.';
				}
		...
		$('.save-settings-button').click(function (event) {
			...
			$.ajax({
				...
				error: function (jqXHR, textStatus, errorThrown) {
					...
					$('.save-settings-button').after('<span class="ajax-success-msg">' + wpsdb_i10n.saved + '</span>');
					$('.ajax-success-msg').fadeOut(2000, function () {
						$(this).remove();
					});
		...
		$('.settings-tab input[type=checkbox]').change(function () {
			...
			$.ajax({
				...
				success: function (data) {
					$('.ajax-spinner').remove();
					$($label).append('<span class="ajax-success-msg">' + wpsdb_i10n.saved + '</span>');
					$('.ajax-success-msg').fadeOut(2000, function () {
						$(this).remove();
					});
				},
		...
		function connection_box_changed(data) {
			...
			$.ajax({
				...
				error: function (jqXHR, textStatus, errorThrown) {
					$('.connection-status').html(wpsdb_i10n.connection_local_server_problem + ' (#100)');
					$('.connection-status').append('<br /><br />Status: ' + jqXHR.status + ' ' + jqXHR.statusText + '<br /><br />Response:<br />' + jqXHR.responseText);
					$('.connection-status').addClass('notification-message error-notice migration-error');
					$('.ajax-spinner').remove();
					doing_ajax = false;
				...
				success: function (data) {
					...
					if ($('#pull').is(':checked')) {
						$('#new-full-url').val(wpsdb_this_url);
